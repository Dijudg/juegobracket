...
export default function UserBackendPage() {
  ...
  const computePodium = useCallback(
    (payload?: BracketSavePayload): PodiumResult => {
      ...
      const officialRoundOf32Matches = migratedPayload.useOfficialRoundOf32Fixtures
        ? ROUND_OF_32_BASE
            .map((cfg): Match | undefined => {
              const fixture = fixtures.find((item) => normalizeMatchKey(item.id) === cfg.id);
              const officialHome = resolveAnyTeam(fixture?.homeId);
              const officialAway = resolveAnyTeam(fixture?.awayId);
              if (!officialHome || !officialAway) return undefined;
              return {
                id: `r32-${cfg.id}`,
                label: cfg.id,
        ...
        : [];
      if (officialRoundOf32Matches.length === ROUND_OF_32_BASE.length) {
    ...
    [buildFullModeGroupsFromScores, buildSelectionsFromPayload, fixtures, resolveAnyTeam, resolveTeamForGroup],
  );
