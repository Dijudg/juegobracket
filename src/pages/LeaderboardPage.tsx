...
const normalizeFixtureIdForRanking = (rawId: unknown, index: number) => {
...
const parseFanaticoFixtures = (fixtures?: Array<Record<string, unknown>>): Fixture[] =>
  (fixtures || []).map((fixture, idx) => ({
    id: normalizeFixtureIdForRanking(fixture.id_partido || fixture.id, idx),
...
const buildFinalMatchesForPodium = (base: Match[], payload: BracketSavePayload) => {
  ...
  const r16 = attachWinners(
    ROUND_OF_16_MAP.map((m) => ({ id: m.id, label: m.label, equipoA: winner(r32, m.a), equipoB: winner(r32, m.b) })),
  );
  const qf = attachWinners(
    QUARTER_FINAL_MAP.map((m) => ({ id: m.id, label: m.label, equipoA: winner(r16, m.a), equipoB: winner(r16, m.b) })),
  );
  const sf = attachWinners(
    SEMI_FINAL_MAP.map((m) => ({ id: m.id, label: m.label, equipoA: winner(qf, m.a), equipoB: winner(qf, m.b) })),
  );
...
const buildOfficialRoundOf32ForPodium = (
  fixtures: Fixture[],
  teamIndex: Map<string, TeamIndexValue>,
): Match[] => {
  return ROUND_OF_32_BASE
    .map((cfg): Match | undefined => {
      const fixture = fixtures.find((item) => normalizeMatchKey(item.id) === cfg.id);
      const home = teamFromToken(fixture?.homeId, undefined, teamIndex);
      const away = teamFromToken(fixture?.awayId, undefined, teamIndex);
...
const buildFullModeCardPodium = (payload: BracketSavePayload, teamIndex: Map<string, TeamIndexValue>, fixtures: Fixture[]) => {
  if (payload.useOfficialRoundOf32Fixtures) {
    const officialMatches = buildOfficialRoundOf32ForPodium(fixtures, teamIndex);
    if (officialMatches.length === ROUND_OF_32_BASE.length) {
      const { final, thirdPlace } = buildFinalMatchesForPodium(officialMatches, payload);
      return {
