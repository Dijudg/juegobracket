...
import localOfficialMatches from "../data/matches.json";
...
import {
  QUARTER_FINAL_MAP,
  ROUND_OF_16_LEFT_DISPLAY_IDS,
  ROUND_OF_16_MAP,
  ROUND_OF_16_RIGHT_DISPLAY_IDS,
  ROUND_OF_32_BASE,
  ROUND_OF_32_LEFT_DISPLAY_IDS,
  ROUND_OF_32_RIGHT_DISPLAY_IDS,
  SEMI_FINAL_MAP,
} from "../features/bracket/officialBracket";
import {
  computeBracketDeadlineState,
  isFixtureLockedByPredictionDeadline,
  isMatchLockedByDeadline,
  isRepechajePhaseArchived,
  resolveFixtureIdFromMatchId,
  type BracketTab,
...
import { ScorePredictionCard, ScorePredictionMatchCard } from "../features/bracket/components/ScorePredictionCard";
import {
  buildConsentPayload,
  clearPendingConsent,
  readPendingConsent,
...
const parseOfficialFixtureNumber = (fixtureId?: string) => {
...
const resolvePredictionIdFromOfficialFixture = (fixtureId?: string) => {
  ...
  const num = parseOfficialFixtureNumber(key);
  if (num === null) return normalizeMatchKey(key);
  if (num <= 72) return `${num}`;
  if (num <= 88) return `r32-${num}`;
  if (num <= 96) return `r16-${num}`;
  if (num <= 100) return `qf-${num}`;
  if (num <= 102) return `sf-${num}`;
  if (num === 103) return "third-103";
  if (num === 104) return "final-104";
  return `P${num}`;
};

const normalizeFixtureIdForGame = (rawId: unknown, index: number) => {
  const value = rawId === undefined || rawId === null ? "" : `${rawId}`.trim();
  const numeric = Number.parseInt(value.replace(/^P/i, ""), 10);
  if (Number.isFinite(numeric) && numeric >= 1 && numeric <= 104) return `${numeric}`;
  return `${index + 1}`;
};
...
const officialMatchesUrl = "https://especiales.eltelegrafo.com.ec/api/matches.json";
...
const LS_GAME_MODE = "fm-game-mode";
...
const isClosedScorePlaceholder = (score?: { home?: number | null; away?: number | null }) =>
  score?.home === CLOSED_SCORE_PLACEHOLDER.home && score.away === CLOSED_SCORE_PLACEHOLDER.away;
...
const UEFA_KEYS = [
  ...
  },
  {
    ...
    mapGroup: "Ruta B",
    semi1: { id: "uefa2-sf1", home: "UKR", away: "SUE", date: "Jueves 23 de marzo" },
...
const buildRoundOf32 = (
  ...
  const matches: Match[] = ROUND_OF_32_BASE.map((cfg) => {
...
const isRoundOf32FixtureNumber = (value?: string) => {
  const num = Number.parseInt(normalizeMatchKey(value), 10);
  return Number.isFinite(num) && num >= 73 && num <= 88;
};

type OfficialMatchRow = Record<string, unknown>;

const unwrapOfficialMatches = (payload: unknown): OfficialMatchRow[] => {
  if (Array.isArray(payload)) return payload as OfficialMatchRow[];
  if (!payload || typeof payload !== "object") return [];
  const record = payload as Record<string, unknown>;
  const candidates = [record.matches, record.fixtures, record.data, record.items];
  const match = candidates.find(Array.isArray);
  return (match || []) as OfficialMatchRow[];
};

const readOfficialToken = (row: OfficialMatchRow, keys: string[]) => {
...
const isPlaceholderTeamLabel = (value?: string) =>
  !value ||
  /\bgrupo\b/i.test(value) ||
  /\bganador\b/i.test(value) ||
  /\bperdedor\b/i.test(value) ||
  /por\s+definir/i.test(value);

const resolveOfficialRoundOf32Number = (row: OfficialMatchRow, index: number) => {
  const explicit = readOfficialToken(row, ["id_partido", "fixtureId", "fixture_id", "matchNumber", "partido"]);
  const explicitNum = Number.parseInt(explicit.replace(/^P/i, ""), 10);
  if (Number.isFinite(explicitNum) && explicitNum >= 73 && explicitNum <= 88) return `${explicitNum}`;

  const inferredNum = index + 1;
  if (inferredNum >= 73 && inferredNum <= 88) return `${inferredNum}`;
  return "";
};
...
export default function BracketGamePage() {
  ...
  const [gameMode, setGameMode] = useState<"classic" | "full">(() => {
    if (typeof window === "undefined") return "classic";
    return window.localStorage.getItem(LS_GAME_MODE) === "full" ? "full" : "classic";
  });
  ...
  const [officialMatches, setOfficialMatches] = useState<OfficialMatchRow[]>(() => unwrapOfficialMatches(localOfficialMatches));
  ...
  useEffect(() => {
    ...
    loadScoreSheetData()
      .then((data) => {
        if (active) setOfficialScoreData(data);
      })
      .catch(() => {
        if (active) setOfficialScoreData(null);
      });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(LS_GAME_MODE, gameMode);
  }, [gameMode]);
  ...
  const reset = () => {
    ...
    setHasLoadedBracketPayload(false);
    setUseOfficialRoundOf32Fixtures(false);
  };
  ...
  }, [selections, bestThirdIds, picks, scorePredictions, penaltyPredictions, useOfficialRoundOf32Fixtures, intercontinentalPicks, uefaPicks, isLocked, phaseLocks, authUser, gameMode]);
  ...
  const applySavedBracket = useCallback(
    (payload: BracketSavePayload) => {
      ...
      setUseOfficialRoundOf32Fixtures(!!migratedPayload.useOfficialRoundOf32Fixtures);
      setHasLoadedBracketPayload(true);
  ...
  const handleSaveClick = async () => {
    ...
    setShowSaveModal(false);
    const autoName = generateBracketCode();
    setSaveMode(authSession?.access_token && currentSaveId ? "update" : "new");
    setSaveName(autoName);
    setSelectedOverwriteId(null);
    if (!authSession?.access_token) {
      ...
      if (currentSaveId) {
        if ((options?.source || "save") === "save") {
          const { error } = await supabase
            .from("bracket_saves")
            .update({ data: payload, name: currentSaveName || name })
            .eq("id", currentSaveId)
            .eq("user_id", userId);
          if (error) throw error;
          setShowSaveModal(false);
          setSaveNotice("Partida guardada correctamente.");
          trackEvent("save_success", {
            mode: "update",
            is_authed: true,
            save_target: "user",
  ...
  useEffect(() => {
    let cancelled = false;
    const loadOfficialMatches = async () => {
      try {
        const response = await fetch(officialMatchesUrl);
        if (!response.ok) return;
        const payload = await response.json();
        const rows = unwrapOfficialMatches(payload);
        if (!cancelled && rows.length > 0) setOfficialMatches(rows);
      } catch {
        // keep bundled fallback
      }
    };
    void loadOfficialMatches();
    return () => {
      cancelled = true;
    };
  }, []);
        ...
        if (apiData?.teams?.length) {
          ...
          if (apiData.fixtures?.length) {
            const parsedFixtures: Fixture[] = apiData.fixtures.map(
              (fixture, idx) => ({
                id: normalizeFixtureIdForGame(fixture.id_partido, idx),
  ...
  const missingMatchLabels = useCallback(
    (round: Match[]) =>
      round
        .filter((m) => (gameMode === "full" ? !m.ganador : !picks[m.id]))
        .map((m) => `Partido ${m.label}`),
    [gameMode, picks],
  );
  ...
  const fullModeGroupTables = useMemo(() => {
    ...
    return tables;
  }, [gameMode, groupFixturesByGroup, groups, nowTs, officialScoreData, resolveTeamForGroup, scorePredictions]);
  ...
  const fullModeThirdStandings = useMemo(() => {
    if (gameMode !== "full") return [];
    return groups
      .map(({ grupo }) => {
        const table = fullModeGroupTables[grupo] || [];
      ...
      .sort((a, b) => {
        ...
        return b.pts - a.pts || b.dif - a.dif || b.gf - a.gf || a.grupo.localeCompare(b.grupo, "es");
      });
  }, [gameMode, groups, fullModeGroupTables]);

  const officialGroupSelections = useMemo<GroupSelections>(() => {
    const next: GroupSelections = {};
    officialScoreData?.groupStandings.forEach((standing, group) => {
      const groupKey = group.toUpperCase();
      next[groupKey] = {
        primero: standing[0]?.teamId ? resolveTeamForGroup(standing[0].teamId, groupKey) : undefined,
        segundo: standing[1]?.teamId ? resolveTeamForGroup(standing[1].teamId, groupKey) : undefined,
        tercero: standing[2]?.teamId ? resolveTeamForGroup(standing[2].teamId, groupKey) : undefined,
      };
    });
    return next;
  }, [officialScoreData, resolveTeamForGroup]);

  const officialBestThirdIds = useMemo(() => {
    if (!officialScoreData || officialScoreData.groupStandings.size < 12) return [];
    return Array.from(officialScoreData.groupStandings.values())
  ...
  const useOfficialGroupsForClassic =
    gameMode !== "full" &&
    phaseDeadlineLocked.grupos &&
    Object.keys(officialGroupSelections).length >= GROUP_LETTERS.length &&
    officialBestThirdIds.length >= MAX_THIRD;

  const groupSelectionSource = useMemo(
    () =>
      gameMode === "full"
        ? fullModeSelections
        : useOfficialGroupsForClassic
        ? officialGroupSelections
        : selections,
    [fullModeSelections, gameMode, officialGroupSelections, selections, useOfficialGroupsForClassic],
  );

  const bracketBestThirdIds = useMemo(
    () =>
      gameMode === "full"
        ? bestThirdIds
        : useOfficialGroupsForClassic
        ...
        : bestThirdIds,
    [bestThirdIds, gameMode, officialBestThirdIds, useOfficialGroupsForClassic],
  );

  const seeds = useMemo(
    () => buildSeedsFromSelections(groupSelectionSource),
    [groupSelectionSource],
  );

  const thirdsAvailable = useMemo(() => {
    const list: Team[] = [];
    const source = groupSelectionSource;
    GROUP_LETTERS.forEach((g) => {
  ...
  const thirdsQualifiedGroups = useMemo(() => {
    ...
    const ordered = bracketBestThirdIds
      .map((id) => map.get(id))
    ...
    return deduped;
  }, [thirdsAvailable, bracketBestThirdIds]);
  ...
  const bestThirdTeams = useMemo(
    ...
    [bracketBestThirdIds, thirdsAvailable],
  );

  const shareDisabled = bracketBestThirdIds.length < MAX_THIRD;
  ...
  const getSeedLabel = (team?: Team) => {
    ...
    if (pick?.tercero?.id === team.id) {
      const order = bracketBestThirdIds.indexOf(team.id);
...
const fixtureByMatchKey = useMemo(() => {
  const map: Record<string, Fixture> = {};
  fixtures.forEach((fx) => {
    const id = normalizeMatchKey(fx.id);
    if (id) map[id] = fx;
...
const findFixtureForMatch = useCallback(
  (matchId?: string, label?: string) => {
    const labelKey = normalizeMatchKey(label);
    const idKey = normalizeMatchKey(matchId);
    return (labelKey && fixtureByMatchKey[labelKey]) || (idKey && fixtureByMatchKey[idKey]) || undefined;
  },
  [fixtureByMatchKey],
);

const findOfficialFixtureForMatch = useCallback(
  (matchId?: string, label?: string): ScoreFixture | undefined => {
    if (!officialScoreData) return undefined;
    const candidates = [
      resolveFixtureIdFromMatchId(label),
      resolveFixtureIdFromMatchId(matchId),
      label ? `P${normalizeMatchKey(label)}` : "",
      matchId ? `P${normalizeMatchKey(matchId)}` : "",
      label ? normalizeMatchKey(label) : "",
      matchId ? normalizeMatchKey(matchId) : "",
    ].filter(Boolean) as string[];
    for (const candidate of candidates) {
      const direct = officialScoreData.fixturesById.get(candidate);
      if (direct) return direct;
      const numeric = normalizeMatchKey(candidate);
      const numericMatch = numeric ? officialScoreData.fixturesById.get(numeric) : undefined;
...
const readOfficialScorePrediction = useCallback(
  (matchId?: string, label?: string) => {
    const fixture = findOfficialFixtureForMatch(matchId, label);
    if (typeof fixture?.homeScore !== "number" || typeof fixture?.awayScore !== "number") return undefined;
    return { home: fixture.homeScore, away: fixture.awayScore };
  },
  [findOfficialFixtureForMatch],
);

const readOfficialPenaltyPrediction = useCallback(
  (matchId?: string, label?: string) => {
    const fixture = findOfficialFixtureForMatch(matchId, label);
    if (typeof fixture?.homePenaltyScore !== "number" || typeof fixture?.awayPenaltyScore !== "number") return undefined;
    return { home: fixture.homePenaltyScore, away: fixture.awayPenaltyScore };
  },
  [findOfficialFixtureForMatch],
);

const isScorePredictionLocked = useCallback(
  (matchId?: string, label?: string) => {
    if (isViewOnly) return true;
    const fixture = findFixtureForMatch(matchId, label);
    return isFixtureLockedByPredictionDeadline(fixture, new Date(nowTs));
  },
  [findFixtureForMatch, isViewOnly, nowTs],
);

const scorePredictionsForBracket = useMemo(() => {
  if (gameMode !== "full") return scorePredictions;
  const next: ScorePredictionState = { ...scorePredictions };
  fixtures.forEach((fixture) => {
    const matchKey = normalizeMatchKey(fixture.id);
    const fixtureId = resolveFixtureIdFromMatchId(fixture.id) || `P${normalizeMatchKey(fixture.id)}`;
    const predictionId = resolvePredictionIdFromOfficialFixture(fixtureId);
    ...
    const current = next[predictionId] || (matchKey ? next[matchKey] : undefined);
    if (hasScorePrediction(current) && (!official || isViewOnly || !isClosedScorePlaceholder(current))) return;
    const resolved = official || current || CLOSED_SCORE_PLACEHOLDER;
    next[predictionId] = resolved;
    if (matchKey && matchKey !== predictionId && !hasScorePrediction(next[matchKey])) {
      next[matchKey] = resolved;
    }
  });
  return next;
}, [fixtures, gameMode, isViewOnly, nowTs, readOfficialScorePrediction, scorePredictions]);

const penaltyPredictionsForBracket = useMemo(() => {
  if (gameMode !== "full") return penaltyPredictions;
  const next: ScorePredictionState = { ...penaltyPredictions };
  fixtures.forEach((fixture) => {
    const matchKey = normalizeMatchKey(fixture.id);
    const fixtureId = resolveFixtureIdFromMatchId(fixture.id) || `P${normalizeMatchKey(fixture.id)}`;
    const predictionId = resolvePredictionIdFromOfficialFixture(fixtureId);
...
const picksForBracket = useMemo(() => {
  ...
  fixtures.forEach((fixture) => {
    const fixtureId = resolveFixtureIdFromMatchId(fixture.id) || `P${normalizeMatchKey(fixture.id)}`;
    const fixtureNumber = parseOfficialFixtureNumber(fixtureId);
    if (fixtureNumber === null || fixtureNumber < 73) return;
    const pickId = resolvePredictionIdFromOfficialFixture(fixtureId);
    if (!pickId || next[pickId]) return;
    if (!isFixtureLockedByPredictionDeadline(fixture, new Date(nowTs))) return;
    const officialFixture = findOfficialFixtureForMatch(pickId, fixture.id);
    const winnerId = officialFixture?.winnerId?.toString().trim().toUpperCase();
    if (!winnerId || winnerId === "EMPATE" || winnerId.includes("/")) return;
    next[pickId] = winnerId;
  });
  return next;
}, [findOfficialFixtureForMatch, fixtures, gameMode, nowTs, picks]);

const officialRoundOf32FixtureMap = useMemo(() => {
  const map = new Map<string, Match>();
  officialMatches.forEach((row, index) => {
    const label = resolveOfficialRoundOf32Number(row, index);
    if (!isRoundOf32FixtureNumber(label)) return;

    const homeName = readOfficialToken(row, ["homeTeamName", "homeTeamShortName", "local", "home"]);
    const awayName = readOfficialToken(row, ["awayTeamName", "awayTeamShortName", "visita", "away"]);
    if (isPlaceholderTeamLabel(homeName) || isPlaceholderTeamLabel(awayName)) return;

    const homeToken = readOfficialToken(row, ["homeTeamCode", "homeTeamId", "local_id", "home_id"]);
    const awayToken = readOfficialToken(row, ["awayTeamCode", "awayTeamId", "visita_id", "away_id"]);
    const officialHome = resolveAnyTeam(homeToken) || resolveAnyTeam(homeName);
    const officialAway = resolveAnyTeam(awayToken) || resolveAnyTeam(awayName);
    if (!officialHome || !officialAway) return;

    map.set(`r32-${label}`, {
      id: `r32-${label}`,
      label,
      equipoA: officialHome,
      equipoB: officialAway,
    });
  });

  return map;
}, [officialMatches, resolveAnyTeam]);

const hasExistingKnockoutPredictions = useMemo(
  () =>
    [picks, scorePredictions, penaltyPredictions].some((state) =>
      Object.keys(state).some((id) => /^(r32|r16|qf|sf|third|final)-/i.test(id)),
    ),
  [penaltyPredictions, picks, scorePredictions],
);

useEffect(() => {
  if (useOfficialRoundOf32Fixtures) return;
  if (isViewOnly || showNewGamePrompt) return;
  if (hasLoadedBracketPayload || hasExistingKnockoutPredictions) return;
  if (!phaseDeadlineLocked.grupos) return;
  setUseOfficialRoundOf32Fixtures(true);
}, [
  hasExistingKnockoutPredictions,
  hasLoadedBracketPayload,
  isViewOnly,
  phaseDeadlineLocked.grupos,
  showNewGamePrompt,
  useOfficialRoundOf32Fixtures,
...
const officialRoundOf32Matches = useMemo(
  () => (useOfficialRoundOf32Fixtures ? officialRoundOf32FixtureMap : new Map<string, Match>()),
  [officialRoundOf32FixtureMap, useOfficialRoundOf32Fixtures],
);
...
const renderPenaltyPicker = (match: Match, side: "home" | "away", label: string, team?: Team) => {
  const selectedValue = penaltyPredictionsForBracket[match.id]?.[side];
  const escudo = getTeamEscudo(team);
  const code = getTeamCode(team) || "--";
  const locked = isMatchBlockedByDeadline(match.id);

  return (
    <div className="penalty-picker">
      <div className="penalty-picker__team">
        <span className="penalty-picker__flag">
          {escudo ? <img src={escudo} alt={team?.nombre || label} /> : <span>{code}</span>}
        </span>
        <span className="penalty-picker__copy">
          <span className="penalty-picker__side">{label}</span>
          <span className="penalty-picker__name">{team?.nombre || "Por definir"}</span>
        </span>
      </div>
      <div className="penalty-picker__dots" role="group" aria-label={`Penales ${label}`}>
        {[1, 2, 3, 4, 5].map((value) => {
          const selected = typeof selectedValue === "number" && selectedValue >= value;
          return (
            <button
              key={`${match.id}-${side}-${value}`}
              type="button"
              disabled={locked}
              onClick={() => handlePenaltyPredictionChange(match.id, side, value)}
              className={`penalty-dot${selected ? " is-selected" : ""}`}
              aria-label={`${value} penal${value === 1 ? "" : "es"} ${label}`}
      ...
      <span className="penalty-picker__value">{typeof selectedValue === "number" ? selectedValue : "-"}</span>
    </div>
  );
};

  const { r32, r16, qf, sf, final, thirdPlace } = useMemo(() => {
    if (useOfficialRoundOf32Fixtures) {
      const officialMatches = ROUND_OF_32_BASE
        .map((cfg) => officialRoundOf32Matches.get(`r32-${cfg.id}`) || {
          id: `r32-${cfg.id}`,
          label: cfg.id,
        });
      setBracketError(undefined);
      return buildNextRounds(officialMatches, picksForBracket, scorePredictionsForBracket, penaltyPredictionsForBracket, gameMode);
    }
    if (bracketBestThirdIds.length < MAX_THIRD) return { r32: [], r16: [], qf: [], sf: [], final: [], thirdPlace: [] };
    ...
    setBracketError(undefined);
    const officialMatches = officialRoundOf32Matches.size
      ? matches.map((match) => officialRoundOf32Matches.get(match.id) || match)
      : matches;
    return buildNextRounds(officialMatches, picksForBracket, scorePredictionsForBracket, penaltyPredictionsForBracket, gameMode);
  }, [seeds, thirdsQualifiedGroups, picksForBracket, bracketBestThirdIds.length, scorePredictionsForBracket, penaltyPredictionsForBracket, gameMode, officialRoundOf32Matches, useOfficialRoundOf32Fixtures]);
  useEffect(() => {
    r32Ref.current = r32;
  }, [r32]);
  ...
  const r32Complete = useMemo(
    () => r32.length > 0 && r32.every((m) => (gameMode === "full" ? !!m.ganador : !!picks[m.id])),
    [gameMode, r32, picks],
  );
  ...
  const r32RightComplete = useMemo(
    () => r32Right.length > 0 && r32Right.every((m) => (gameMode === "full" ? !!m.ganador : !!picks[m.id])),
    [gameMode, r32Right, picks],
  );
  const finalComplete = useMemo(
    () => final.length > 0 && final.every((m) => (gameMode === "full" ? !!m.ganador : !!picks[m.id])),
    [gameMode, final, picks],
  );
  const sfComplete = useMemo(
    () => sf.length > 0 && sf.every((m) => (gameMode === "full" ? !!m.ganador : !!picks[m.id])),
    [gameMode, sf, picks],
  );
  ...
  const autoSwitchLlavesRef = useRef(false);
    useEffect(() => {
      ...
      if (isViewOnly) return;
      if ((!groupCompletion.complete || !thirdsComplete) && !phaseDeadlineLocked.grupos) {
        autoSwitchLlavesRef.current = false;
        return;
      }
    if (activeTab === "dieciseisavos") return;
    if (!autoSwitchLlavesRef.current) {
      setActiveTab("dieciseisavos");
      autoSwitchLlavesRef.current = true;
    }
    }, [groupCompletion.complete, thirdsComplete, phaseDeadlineLocked.grupos, activeTab, showNewGamePrompt, isViewOnly, defaultStartTab]);
      ...
      for (let i = 0; i < list.length; i += 2) {
        ...
        items.push(
            <DieciseisavosKeyBlock
              ...
              onPenaltyClick={gameMode === "full" ? setPenaltyModalMatch : undefined}
            />,
        );
      }
      return items;
  ...
  return (
    ...
    >
      ...
      <main className="max-w-7xl px-2 sm:px-6 lg:px-10 xl:px-16">
          <div className="max-w-7xl mx-auto">
            ...
            {!isViewOnly && (
              <>
                ...
                {!showModeHome && <div className="flex flex-col gap-3 md:flex-row md:items-end">
                  ...
                  {authSession?.access_token && (
                    <button
                      type="button"
                      onClick={handleSaveClick}
                      disabled={saveBusy || confirmSaveInFlightRef.current || pendingAuthSaveRef.current}
                      className={`inline-flex min-h-11 items-center justify-center rounded-md px-5 text-sm font-black uppercase transition ${
                        saveBusy || confirmSaveInFlightRef.current || pendingAuthSaveRef.current
                ...
                {showModeHome && (
                <section
                  className="game-mode-home"
                  style={{ ["--mode-home-bg" as any]: `url(${portadaBanner})` }}
                  aria-label="Seleccionar modo de juego"
                >
                  <div className="game-mode-home__header">
                    <div className="game-mode-home__laurel" aria-hidden="true">‹</div>
                    <div className="min-w-0">
                      <h1 className="game-mode-home__title">
                        <span>El Mundial</span>
                        <strong>es tuyo</strong>
                      </h1>
                      <p className="game-mode-home__subtitle">Elige tu modo de juego y arma tu camino hacia la gloria.</p>
                      {!authSession?.access_token && (
                        <div className="game-mode-home__auth">
                          <button
                            type="button"
                            onClick={() => openAuthModal("signup")}
                            className="game-mode-home__auth-button game-mode-home__auth-button--primary"
                          >
                            Registrarse
                          </button>
                          <button
                            type="button"
                            onClick={() => openAuthModal("login")}
                            className="game-mode-home__auth-button"
                          >
                            Iniciar sesión
                          </button>
                        </div>
                      ...
                      {authSession?.access_token && (
                        <p className="game-mode-home__session">Sesión iniciada. Elige cómo quieres jugar.</p>
                      )}
                    </div>
                  ...
                  <div className="game-mode-home__cards">
                    ...
                    >
                      <span className="game-mode-card__icon"><ClipboardList /></span>
                      <span className="game-mode-card__title">Clásico</span>
                      <span className="game-mode-card__subtitle">Modo tradicional de predicción.</span>
                      <span className="game-mode-card__feature">
                        <BarChart3 />
                        <Star />
                        <span>Clasificados, llaves y campeón.</span>
                      </span>
                      <span className="game-mode-card__cta">
                        Jugar modo clásico
                        <ChevronRight />
                      </span>
                    </button>
                    ...
                    >
                      <span className="game-mode-card__icon"><Trophy /></span>
                      <span className="game-mode-card__title">Completo</span>
                      <span className="game-mode-card__subtitle">Predice todo y suma más puntos.</span>
                      <span className="game-mode-card__feature">
                        <BarChart3 />
                        <Star />
                        <span>Marcadores por partido, clasificados y bonos por fase.</span>
                      </span>
                      <span className="game-mode-card__cta">
                        Jugar modo completo
                        <ChevronRight />
          ...
          {!showModeHome && !isEmbedded && (
            ...
            {gameMode === "full" && (
              <a
                ...
                className="ml-1 inline-flex items-center gap-2 rounded-b-xl rounded-t-none border-l border-neutral-800 px-3 py-2 whitespace-nowrap text-base font-semibold text-gray-400 transition hover:text-white"
              >
                <Info className="size-4 text-[#c6f600]" />
                Reglas
              </a>
            )}
            </div>
            ...
            {!showModeHome && !isViewOnly && saveNotice && (
          ...
          {!showModeHome && (<>
          <AnimatePresence>
              {!showModeHome && showGuestCodeSticky && guestSharePanel && (
                <motion.div
          ...
          <AnimatePresence mode="wait">
            ...
            {gameMode === "full" ? (
              <div ref={progressGroupsRef} className="full-groups-layout">
                <div className="full-group-tabs" role="tablist" aria-label="Grupos">
                  {groups.map(({ grupo }) => (
                    <button
                      key={`full-tab-${grupo}`}
                      type="button"
                      ...
                      onClick={() => {
                        ...
                        setFullGroupDirection(nextIndex >= currentIndex ? 1 : -1);
                        setActiveFullGroup(grupo);
                      }}
                      className={`full-group-tab ${currentFullGroup === grupo ? "is-active" : ""}`}
                      aria-selected={currentFullGroup === grupo}
                    >
                      {grupo}
                    </button>
                  ))}
                ...
                {activeFullGroupData && (
                  <section
                    key={currentFullGroup}
                    id={`full-group-${currentFullGroup}`}
                  ...
                  >
                    <div className="full-group-matches">
                      {(groupFixturesByGroup[currentFullGroup] || []).map((fixture) => {
                        const home = resolveTeamForGroup(fixture.homeId, currentFullGroup);
                        const away = resolveTeamForGroup(fixture.awayId, currentFullGroup);
                        const matchKey = normalizeMatchKey(fixture.id);
                        return (
                          <ScorePredictionCard
                            key={`group-score-${fixture.id}`}
                            matchId={matchKey}
                    ...
                    <div className="full-group-table-wrap">
                      <div className="full-group-title-row">
                        <button
                          type="button"
                          className="full-group-arrow"
                          aria-label="Grupo anterior"
                          onClick={() => navigateFullGroup(-1)}
                        >
                          ‹
                        </button>
                        <h2>Grupo {currentFullGroup}</h2>
                        <button
                          type="button"
                          className="full-group-arrow"
                      ...
                      <table className="full-group-table">
                        <thead>
                          <tr>
                            <th>#</th>
                            <th>Seleccion</th>
          ...
          {missingThirdGroups.length > 0 && (
            <div className="mt-4 text-xl text-center text-gray-300">
              {gameMode === "full" ? "Te faltan completar marcadores hasta el grupo:" : "Te faltan seleccionar el tercero del grupo:"}{" "}
          ...
          >
            ...
            {bestThirdTeams.length === 0 && gameMode !== "full" ? (
              <p className="text-sm text-gray-400">Aun no seleccionas terceros.</p>
            ) : (
              <div className="w-full">
                {gameMode === "full" && fullModeThirdStandings.length > 0 && (
                  <div className="full-thirds-table-wrap">
                    <table className="full-thirds-table">
                      <thead>
                        <tr>
                          <th>#</th>
                          <th>Grupo</th>
                      ...
                      <tbody>
                        {fullModeThirdStandings.map((entry, idx) => {
                          const qualifies = entry.complete && !!entry.team && bestThirdIds.includes(entry.team.id);
                          const escudo = getTeamEscudo(entry.team);
                          return (
                            <tr
                              key={`full-third-${entry.grupo}`}
                              className={`${qualifies ? "is-qualified" : ""} ${!entry.complete ? "is-pending" : ""}`}
                            >
                              <td>{entry.complete ? idx + 1 : "--"}</td>
                              <td>Grupo {entry.grupo}</td>
                              <td>
                                <span className="full-thirds-team">
                                  <span className="full-thirds-team__flag">
                                    {escudo ? <img src={escudo} alt={entry.team?.nombre || entry.grupo} /> : entry.team?.codigo || "--"}
                                  </span>
                                  <span>{entry.team?.nombre || "Pendiente"}</span>
                                </span>
                              </td>
                              <td>{entry.complete ? entry.pts : "--"}</td>
                              <td>{entry.complete ? entry.pj : "--"}</td>
                              <td>{entry.complete ? entry.dif : "--"}</td>
                              <td>{entry.complete ? entry.gf : "--"}</td>
                              <td>
                                <span className={`full-thirds-status ${qualifies ? "is-qualified" : entry.complete ? "" : "is-pending"}`}>
                                  {qualifies ? "Clasifica" : entry.complete ? "Fuera" : "Pendiente"}
                                </span>
                              </td>
                            </tr>
                          );
      ...
      {scoreModalMatch && gameMode === "full" && !showModeHome && !showNewGamePrompt && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4 py-6"
          onClick={() => setScoreModalMatch(null)}
        >
          <div
            className="w-full md:w-1/2 max-w-3xl rounded-lg border border-neutral-700 bg-neutral-950 p-4 shadow-2xl"
            onClick={(event) => event.stopPropagation()}
            role="dialog"
            aria-modal="true"
          >
            <div className="mb-3 flex items-center justify-between">
              <h3 className="text-xl font-black text-[#c6f600]">Marcador del partido</h3>
              <button
                type="button"
                onClick={() => setScoreModalMatch(null)}
            ...
            <ScorePredictionMatchCard
              match={scoreModalMatch}
              schedule={scheduleByMatch[normalizeMatchKey(scoreModalMatch.label) || normalizeMatchKey(scoreModalMatch.id)]}
              value={scorePredictionsForBracket[scoreModalMatch.id]}
            ...
            scorePredictions[scoreModalMatch.id]!.home === scorePredictions[scoreModalMatch.id]!.away ? (
              <button
                ...
                onClick={() => {
                  setScoreModalMatch(null);
                  setPenaltyModalMatch(scoreModalMatch);
                }}
                className="mt-3 w-full rounded-md border border-[#c6f600]/60 bg-[#c6f600] px-3 py-2 text-sm font-black text-black hover:brightness-95"
              >
                Editar penales
                {hasScorePrediction(penaltyPredictions[scoreModalMatch.id])
                  ? ` (${penaltyPredictions[scoreModalMatch.id]!.home}-${penaltyPredictions[scoreModalMatch.id]!.away})`
                  : ""}
              </button>
            ) : null}
          </div>
          ...
          >
            ...
            <div className="grid gap-4">
              <div className="grid gap-3 md:grid-cols-2">
                {renderPenaltyPicker(penaltyModalMatch, "home", "Local", penaltyModalMatch.equipoA)}
                {renderPenaltyPicker(penaltyModalMatch, "away", "Visitante", penaltyModalMatch.equipoB)}
              </div>
      ...
      {registeredSavePopup && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/75 px-4 py-6">
          <div className="w-full max-w-md rounded-xl border border-[#c6f600]/60 bg-neutral-950 p-5 text-center shadow-[0_0_34px_rgba(198,246,0,0.22)]">
            <div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-full bg-[#c6f600] text-black">
    ...
    setActiveTab(defaultStartTab);
        ...
        />
        <R32InfoModal
          open={showR32Warning && !showModeHome && !showNewGamePrompt && !isViewOnly}
          title="Calma"
        ...
        <SemifinalPromptModal
          open={showSemifinalPrompt && !showModeHome && !showNewGamePrompt && !isViewOnly}
        ...
        />
        <SaveModal
          open={showSaveModal && !showModeHome && !showNewGamePrompt && !isViewOnly}
        ...
        />
        <PhaseBlockModal
          open={!!phaseBlock && !showModeHome && !showNewGamePrompt && !isViewOnly}
          title={phaseBlock?.title || ""}
          missing={phaseBlock?.missing || []}
          onClose={() => setPhaseBlock(null)}
        />
        <AnimatePresence>
          {showIntercontinentalModal && !showModeHome && !showNewGamePrompt && !isViewOnly && (
          <motion.div
